...
- **Tone specialist** — compiles the engine-neutral intent into engine params. In
  v0.1 this is the deterministic [tone table](configuration.md#voice_profilesyaml-the-tone-table);
...
## Troubleshooting — complaint → stage
...
| right words, wrong tone or pace | **Tone intent** or the table | adjust the beat's `{tone}` / edit [`voice_profiles.yaml`](configuration.md#voice_profilesyaml-the-tone-table) |
| a name is mispronounced / digits read wrong | **compile** (normalize/lexicon) | add a [`lexicon.yaml`](configuration.md#lexiconyaml-pronunciation) entry |
